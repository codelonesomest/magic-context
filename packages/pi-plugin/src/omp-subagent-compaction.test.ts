...
describe("OMP task subagent lifecycle", () => {
	...
	it("classifies a child from OMP's fixed-width title slot session files", () => {
		const db = createTestDb();
		const sessionDirectory = mkdtempSync(
			join(tmpdir(), "mc-omp-task-session-slot-"),
		);
		try {
			const sessionFile = join(sessionDirectory, "ChildAgent.jsonl");
			const titleSlot = `${JSON.stringify({
				type: "title",
				v: 1,
				title: "",
			...
			writeFileSync(
				sessionFile,
				`${titleSlot}${JSON.stringify({ type: "session", id: "ses-omp-child-slot" })}\n`,
			);
			let lifecycleHandler: ((payload: unknown) => void) | undefined;
			const pi = {
				events: {
			...
			expect(getOrCreateSessionMeta(db, "ses-omp-child-slot").isSubagent).toBe(
				true,
			);
		} finally {
			closeQuietly(db);
